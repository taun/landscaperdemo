...
@implementation NSObject (AppBotKeychainPropertyListCoding)

- (id)AppBotKeychain_propertyListRepresentation
{
    return self;
}
...
#if !APPBOTKEYCHAIN_USE_NSCODING

@implementation NSNull (AppBotKeychainPropertyListCoding)
...
- (id)AppBotKeychain_propertyListRepresentation
{
    ...
    [self enumerateObjectsUsingBlock:^(__unsafe_unretained id obj, __unused NSUInteger idx, __unused BOOL *stop) {
        id value = [obj AppBotKeychain_propertyListRepresentation];
...
+ (instancetype)defaultKeychain
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedInstance = [[ABXKeychain alloc] initWithService:bundleID
                                                      accessGroup:nil];
    });
          ...
          accessGroup:(NSString *)accessGroup
{
    return [self initWithService:service
                   ...
                   accessibility:ABXKeychainAccessibleWhenUnlocked];
}
...
- (NSData *)dataForKey:(id)key
{
    ...
    {
		NSLog(@"AppBotKeychain failed to retrieve data for key '%@', error: %ld", key, (long)status);
	}
...
- (BOOL)setObject:(id)object forKey:(id)key
{
    ...
    if (object && !data)
    {
        data = [NSPropertyListSerialization dataWithPropertyList:[object AppBotKeychain_propertyListRepresentation]
    ...
    NSAssert(!object || (object && data), @"AppBotKeychain failed to encode object for key '%@', error: %@", key, error);
    ...
    if (data)
    {
        ...
        {
            NSLog(@"AppBotKeychain failed to store data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    else
    {
        ...
        if (status != errSecSuccess)
        {
            NSLog(@"AppBotKeychain failed to delete data for key '%@', error: %ld", key, (long)status);
            return NO;
        }
    }
    return YES;
...
- (id)objectForKey:(id)key
{
    ...
    if (data)
    {
        ...
        if (!object)
        {
            NSLog(@"AppBotKeychain failed to decode data for key '%@', error: %@", key, error);
        }
        return object;
    }
    else
